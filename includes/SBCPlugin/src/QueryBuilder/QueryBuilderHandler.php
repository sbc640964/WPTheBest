<?php namespace SBCPlugin\QueryBuilder;

use SBCPlugin\Connection;
use SBCPlugin\Exception;
...
class QueryBuilderHandler
{
    ...
    /**
     * @var \SBCPlugin\QueryBuilder\Adapters\BaseAdapter
     */
    ...
    /**
     * @param null|\SBCPlugin\Connection $connection
     *
     * @throws \SBCPlugin\Exception
     */
    public function __construct(Connection $connection = null)
    {
        ...
        $this->adapterInstance = $this->container->build(
            '\\SBCPlugin\\QueryBuilder\\Adapters\\' . ucfirst($this->adapter),
    ...
    public function query($sql, $bindings = array())
    {
        $this->dbStatement = $this->container->build(
            '\\SBCPlugin\\QueryBuilder\\QueryObject',
            array($sql, $bindings)
        )->getRawSql();
    ...
    {
        $allowedTypes = array(
            'select', 'insert', 'insertignore', 'replace', 'delete', 'update', 'criteriaonly'
        );
    ...
    {
        $result = $this->query('SELECT ROW_COUNT() as affected_row_count')->get();

        if ($result && isset($result[0])) {
            return $result[0]->affected_row_count;
        }
    ...
    {
        if(empty($values)) {
            return $this->where($this->raw('0 = 1'));
        }
    ...
    public function join($table, $key, $operator = null, $value = null, $type = 'inner')
    {
        ...
        $joinBuilder = $this->container->build('\\SBCPlugin\\QueryBuilder\\JoinBuilder', array($this->connection));
    ...
    public function transaction(\Closure $callback)
    {
        try {
            ...
            $transaction = $this->container->build(
                '\\SBCPlugin\\QueryBuilder\\Transaction',
                array($this->connection)
            );
    ...
    {
        return $this->container->build('\\SBCPlugin\\QueryBuilder\\Raw', array($value, $bindings));
    }
    ...
    public function paginate($perPage = null, $columns = array('*'))
    {
        ...
        $perPage = $perPage ?: (isset($req['per_page']) ? intval($req['per_page']) : 15);
